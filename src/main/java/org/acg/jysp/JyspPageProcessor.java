...
public class JyspPageProcessor implements PageProcessor {
    private final Site site = Site.me().setDomain("www.chsi.com.cn");

    @Override
    public void process(Page page) {
        FileWriter fileWriter = new FileWriter("D:\\temp\\jysp.txt");
        ...
        liNodes.forEach(item -> {
            ...
            String link = "/jysp/" + date.substring(0, date.indexOf("-")) + "/" + url.substring(url.lastIndexOf("/") + 1, url.lastIndexOf(".")) + ".html";
            paper.setLink(link);

            fileWriter.append(JSONUtil.toJsonStr(paper) + ",");

            ThreadUtil.execAsync(() -> {
                JyspPaperProcessor.buildPaper("https://" + site.getDomain() + url);
            });
        });
    }
